...
import com.gaodeamap.test.demo.util.Const;
 ...
 * on 2020/9/10 11:03 AM
 *
 *   圆形栅栏
 */
 public  class GeoFenceRoundNewActivity  extends AppCompatActivity implements
     ...
     private CheckBox cbAlertStated;
        ...
        public void handleMessage(@NonNull Message msg) {
            switch (msg.what){
                case 0:
                    ...
                    sb.append("添加围栏成功");
                    String customerId = (String) msg.obj;
                ...
                case 1:
                    ...
                    Toast.makeText(getApplicationContext(), "添加栅栏失败："  + errorCode, Toast.LENGTH_SHORT).show();
                    break;
                case 2:
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_geofence_round_new);
        ...
        cbAlertStated = findViewById(R.id.cb_alertStated);
    ...
    private void init(){
        ...
        fileter.addAction(GEOFENCE_BROADCAST_ACTION);
        registerReceiver(mGeoFenceReceiver,fileter);
    ...
    private void setUpMap() {
        ...
        mAmap.getUiSettings().setMyLocationButtonEnabled(true); //设置定位按钮是否显示
        //自定义系统定位蓝点
...
//        mAmap.moveCamera(CameraUpdateFactory.newLatLngBounds(bounds,150));
    ...
    private void drawCircle(GeoFence fence) {
        LatLng latLng = new LatLng(fence.getCenter().getLatitude(),
    ...
    private void addRoundFence() {
        ...
        if (centerLatLng == null || TextUtils.isEmpty(radiusStr)){
    ...
    public void onMapClick(LatLng latLng) {
        ...
        tvGuide.setBackgroundColor(getResources().getColor(R.color.grey));
    ...
    /**
     * 接收触发围栏后的广播，当添加围栏成功之后，会立即对所有围栏状态进行一次侦测，如果当前状态与用户设置的触发行为
     * 相符合将会立即触发一次围栏广播，只有当触发围栏之后才会收到广播，对于同一触发行为只会发送一次广播不会重复发送，
     * 除非位置和围栏的关系再次发生了改变
     * */
    private BroadcastReceiver mGeoFenceReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(GEOFENCE_BROADCAST_ACTION)){
                ...
                String custonId = bundle.getString(GeoFence.BUNDLE_KEY_CUSTOMID);
                String fenceId = bundle.getString(GeoFence.BUNDLE_KEY_FENCEID);
                //status 标识的是当前的围栏状态，不是围栏行为
                ...
                if (status != GeoFence.STATUS_LOCFAIL){
                    if (!TextUtils.isEmpty(custonId)){
                        sb.append("customId:" + custonId);
                    }
